...
class Emoji
{
    protected $GLOBALS = ['emoji_maps' => array(
        'names' => array(
    ...
    {
        $this->GLOBALS['emoji_maps']['html_to_unified'] = array_flip($this->GLOBALS['emoji_maps']['unified_to_html']);
    }
...
#
    ...
    {
        return preg_replace_callback($this->GLOBALS['emoji_maps']['unified_rx'], function ($m) {
            if (isset($m[2]) && $m[2] == "\xEF\xB8\x8E") return $m[0];
            $cp = $this->GLOBALS['emoji_maps']['unified_to_html'][$m[1]];
    ...
    function emoji_html_to_unified($text)
    {
        return preg_replace_callback("!<span class=\"emoji-outer emoji-sizer\"><span class=\"emoji-inner emoji([0-9a-f]+)\"></span></span>!", function ($m) {
            if (isset($this->GLOBALS['emoji_maps']['html_to_unified'][$m[1]])) {
                return $this->GLOBALS['emoji_maps']['html_to_unified'][$m[1]];
            }
            return $m[0];
        }, $text);
    }
    ...
    function emoji_convert($text, $map)
    {
        ...
        return str_replace(array_keys($this->GLOBALS['emoji_maps'][$map]), $this->GLOBALS['emoji_maps'][$map], $text);
    }

    function emoji_get_name($unified_cp)
    {

        return $this->GLOBALS['emoji_maps']['names'][$unified_cp] ? $this->GLOBALS['emoji_maps']['names'][$unified_cp] : '?';
    }
    ...
    function emoji_contains_emoji($text)
    {
        ...
        $count = 0;
        str_replace($this->GLOBALS['emoji_maps']['prefixes'], '00', $text, $count);
        return $count > 0;
    }
}
